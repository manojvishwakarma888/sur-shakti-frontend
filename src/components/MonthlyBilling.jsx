...
export default function MonthlyBilling({ onSuccess }) {
  ...
  return <section>
    <h3 className="h5">Monthly billing</h3>
    <p className="text-muted small">Preview charges before generating. Each charge type creates a separate bill. Existing bills are skipped.</p>
    <ErrorMessage error={error} />
    {result && <div className="alert alert-success" role="status">Created {result.created} bills; skipped {result.skipped} existing bills.</div>}
    <form onSubmit={previewBills} onChange={invalidate}>
      <fieldset disabled={busy}>
        <div className="maintenance-form-grid">
          <Field label="Billing month"><input className="form-control" type="month" value={period} required onChange={e => setPeriod(e.target.value)} /></Field>
          <Field label="Due date"><input className="form-control" type="date" min={period + '-01'} value={dueDate} required onChange={e => setDueDate(e.target.value)} /></Field>
        </div>
        <label className="d-flex gap-2 my-3"><input type="checkbox" checked={allFlats} onChange={e => setAllFlats(e.target.checked)} />Bill all known row houses</label>
        {!allFlats && <Field label="Row house numbers (comma separated)"><textarea className="form-control" value={flats} onChange={e => setFlats(e.target.value)} placeholder="A-101, A-102" required /></Field>}
        {charges.map((charge, index) => <div key={index} className="maintenance-charge-row">
          ...
          <button type="button" className="btn btn-outline-danger" disabled={charges.length === 1} onClick={() => { invalidate(); setCharges(charges.filter((_, i) => i !== index)); }}>Remove</button>
        </div>)}
        <div className="d-flex gap-2 my-3"><button type="button" className="btn btn-outline-primary" onClick={() => { invalidate(); setCharges([...charges, { chargeType: '', amount: '' }]); }}>Add charge</button><button className="btn btn-primary" type="submit">{busy ? 'Working…' : 'Preview bills'}</button></div>
      </fieldset>
    </form>
    {preview && <div>
      <h4 className="h6">Preview: {preview.rows.length} charges</h4>
      <div className="maintenance-table"><table className="table"><thead><tr><th>Row house</th><th>Charge</th><th>Amount</th><th>Result</th></tr></thead><tbody>
        {preview.rows.map((row, i) => <tr key={i}><td>{row.flatNo}</td><td>{row.chargeType}</td><td>{money(row.amount)}</td><td>{row.alreadyExists ? 'Already exists — skip' : 'New bill'}</td></tr>)}
      </tbody></table></div>
      <button className="btn btn-success" disabled={busy || !preview.rows.some(r => !r.alreadyExists)} onClick={generate}>{busy ? 'Generating…' : 'Generate previewed bills'}</button>
    </div>}
  </section>;
}
