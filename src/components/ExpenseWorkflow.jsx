...
export default function ExpenseWorkflow({ refreshVersion = 0, onChanged }) {
  ...
  return <section className="card p-3 p-md-4 mt-4 rounded-4 maintenance-screen">
    <h3 className="h5">Expense approval & bank matching</h3>
    <ErrorMessage error={error} />{message && <p className="alert alert-success" role="status">{message}</p>}
    <button className="btn btn-outline-primary align-self-start mb-3" disabled={busy} onClick={() => setVersion(v => v + 1)}>Refresh workflow</button>
    {busy && <p role="status">Loading…</p>}
    {!busy && !error && !expenses.length && <p>No expenses on this page.</p>}
    {expenses.map(exp => <article key={exp.id} className="border rounded-3 p-3 mb-3">
      ...
      {exp.reviewReason && <p className="small mt-2">Review: {exp.reviewReason}</p>}
      {exp.invoiceUrl && <button className="btn btn-outline-secondary my-2" onClick={() => openAuthenticatedFile(exp.invoiceUrl).catch(err => setError(getApiErrorMessage(err)))}>View invoice</button>}
      {exp.status === 'Submitted' && <div className="mt-2"><Field label={'Review reason for expense ' + exp.id}><input className="form-control" maxLength={500} value={notes[exp.id] || ''} onChange={e => setNotes({ ...notes, [exp.id]: e.target.value })} /></Field><div className="d-flex gap-2 mt-2"><button className="btn btn-success" disabled={busy} onClick={() => review(exp, true)}>Approve</button><button className="btn btn-outline-danger" disabled={busy} onClick={() => review(exp, false)}>Reject</button></div></div>}
      {exp.status === 'Approved' && <div className="mt-2">
        {banks.some(b => b.expenseId === exp.id) ? <p className="text-success">Matched to bank record {banks.find(b => b.expenseId === exp.id).reference}</p> : <>
          <Field label={'Bank record for expense ' + exp.id}><select className="form-select" value={matches[exp.id] || ''} onChange={e => setMatches({ ...matches, [exp.id]: e.target.value })}><option value="">Select a bank payment not yet linked</option>{banks.filter(b => !b.expenseId && Number(b.amount) === Number(exp.amount)).map(b => <option key={b.id} value={b.id}>{b.reference} · {money(b.amount)}</option>)}</select></Field>
          <button className="btn btn-outline-primary mt-2" disabled={busy || !matches[exp.id]} onClick={() => mutate(() => api.post('/maintenance/expenses/' + exp.id + '/match/' + matches[exp.id], undefined, noRetry), 'Bank record matched.')}>Link bank payment</button>
        </>}
      </div>}
      {exp.status !== 'Cancelled' && <button className="btn btn-outline-danger mt-3" disabled={busy || banks.some(b => b.expenseId === exp.id)} onClick={() => cancel(exp)}>Cancel expense</button>}
    </article>)}
    <Pager skip={skip} count={expenses.length} busy={busy} onPage={setSkip} />
    <h4 className="h6 mt-4">Money paid from the bank</h4><p className="small text-muted">Add each bank payment and link it to the matching approved expense.</p>
    <div className="maintenance-table"><table className="table"><thead><tr><th>Reference</th><th>Date</th><th>Amount</th><th>Expense</th></tr></thead><tbody>{banks.map(b => <tr key={b.id}><td>{b.reference}</td><td>{b.date?.slice(0, 10)}</td><td>{money(b.amount)}</td><td>{b.expenseId || 'Not linked'}</td></tr>)}</tbody></table></div>
    ...
    <form className="mt-3" onSubmit={e => { e.preventDefault(); mutate(async () => { await api.post('/maintenance/expenses/bank-records', { ...bank, reference: bank.reference.trim(), amount: Number(bank.amount) }, noRetry); setBank({ reference: '', date: '', amount: '' }); }, 'Bank record saved.'); }}><fieldset disabled={busy}><div className="maintenance-form-grid">
      <Field label="Bank reference"><input className="form-control" required maxLength={100} value={bank.reference} onChange={e => setBank({ ...bank, reference: e.target.value })} /></Field>
      <Field label="Bank date"><input type="date" className="form-control" required value={bank.date} onChange={e => setBank({ ...bank, date: e.target.value })} /></Field>
      <Field label="Amount paid (₹)"><input type="number" min=".01" step=".01" className="form-control" required value={bank.amount} onChange={e => setBank({ ...bank, amount: e.target.value })} /></Field>
    </div><button className="btn btn-primary mt-3">Add bank record</button></fieldset></form>
  </section>;
}
