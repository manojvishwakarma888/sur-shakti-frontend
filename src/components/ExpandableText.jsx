...
export default function ExpandableText({ text = '', limit = 280 }) {
  ...
  return <div className="readable-copy">
    ...
    {long && <button type="button" className="btn btn-link readable-copy-toggle" aria-expanded={expanded} aria-controls={id} onClick={() => setExpanded(value => !value)}>{expanded ? uiText("Show less") : uiText("Read full notice")}<span aria-hidden="true">{expanded ? ' −' : ' +'}</span></button>}
  </div>;
}
